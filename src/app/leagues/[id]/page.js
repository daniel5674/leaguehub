...
export default function LeagueDetailsPage() {
  ...
  useEffect(() => {
    ...
    if (league.leagueType === "personal" && activeTab === "teams") {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-slate-100 to-white px-6 py-10">
      <div className="mx-auto max-w-6xl">
        ...
        {/* Match Day CTA — personal leagues only */}
        {isPersonalLeague && canManage && (
          <div className="mb-6">
            <button
              type="button"
              onClick={() => router.push(`/leagues/${id}/match-day`)}
              className="w-full rounded-3xl bg-gradient-to-r from-emerald-600 to-emerald-700 py-5 text-lg font-black text-white shadow-lg shadow-emerald-600/20 transition hover:from-emerald-500 hover:to-emerald-600 active:scale-[0.99]"
            >
              ⚽ יום משחק
            </button>
          </div>
        ...
        {isPersonalLeague && (
          <section className="mb-8 rounded-3xl border border-purple-200 bg-purple-50 p-6">
            ...
            <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="text-2xl font-bold text-purple-900">שחקני הליגה</h2>
                <p className="mt-1 text-sm text-purple-600">{league.personalPlayers?.length || 0} שחקנים רשומים</p>
              </div>
            </div>

            {/* Player table */}
            {!league.personalPlayers || league.personalPlayers.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-purple-300 bg-white p-8 text-center text-gray-400">
                אין שחקנים עדיין — לחץ על ⚙️ ניהול בתפריט להוספה
              </div>
            ) : (
                ...
                <table className="w-full text-right">
                  <thead>
                    <tr className="border-b border-purple-100 bg-purple-50">
                      <th className="px-4 py-3 text-xs font-semibold text-purple-400">#</th>
                      <th className="px-4 py-3 text-xs font-semibold text-purple-400">שחקן</th>
                      <th className="px-4 py-3 text-xs font-semibold text-purple-400">דירוג</th>
                      {canManage && <th className="px-4 py-3" />}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-purple-50">
                    {league.personalPlayers.map((player, index) => (
                      <tr key={player._id || index} className="transition hover:bg-purple-50/50">
                        <td className="px-4 py-3 text-sm text-purple-300">{index + 1}</td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm font-semibold text-gray-800">{player.fullName}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3 text-left">
                            <button
                              ...
                              onClick={() => openConfirmModal({
                                title: "הסרת שחקן",
                                message: `להסיר את ${player.fullName}?`,
                                onConfirm: async () => { await handleRemovePersonalPlayer(player._id); closeConfirmModal(); },
                              })}
            ...
            {league.generatedTeams?.length > 0 && (
              <div className="mt-8">
                <h3 className="mb-4 text-lg font-black text-gray-900">⚽ כוחות אחרונים</h3>
                <div className="grid gap-3 md:grid-cols-2">
                  {league.generatedTeams.map((team, ti) => {
                    const palette = [
                      { header: "bg-violet-600", badge: "bg-violet-100 text-violet-700", row: "hover:bg-violet-50" },
                      { header: "bg-emerald-600", badge: "bg-emerald-100 text-emerald-700", row: "hover:bg-emerald-50" },
                      { header: "bg-blue-600",    badge: "bg-blue-100 text-blue-700",    row: "hover:bg-blue-50"    },
                      { header: "bg-rose-600",    badge: "bg-rose-100 text-rose-700",    row: "hover:bg-rose-50"    },
                      { header: "bg-amber-600",   badge: "bg-amber-100 text-amber-700",  row: "hover:bg-amber-50"   },
                      { header: "bg-cyan-600",    badge: "bg-cyan-100 text-cyan-700",    row: "hover:bg-cyan-50"    },
                    ];
                    const c = palette[ti % palette.length];
                    return (
                      <div key={team._id || team.name} className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                        <div className={`${c.header} px-5 py-3 text-center text-base font-black text-white`}>
                          {team.name}
                          <span className="mr-2 text-xs font-normal opacity-70">({team.players.length} שחקנים)</span>
                        </div>
                        <div className="divide-y divide-gray-50">
                          {team.players.map((player, pi) => (
                            <div key={`${player.fullName}-${pi}`} className={`flex items-center justify-between px-4 py-2.5 transition ${c.row}`}>
                              <span className="text-sm font-medium text-gray-800">{player.fullName}</span>
                              <span className={`rounded-full px-2.5 py-0.5 text-xs font-bold ${c.badge}`}>{player.rating}</span>
                            </div>
                          ))}
                        </div>
                      </div>
